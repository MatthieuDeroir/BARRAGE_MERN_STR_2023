...
async function getDisplaySettingsWithPlaylistItems(req, res) {
  try {
    const displaySettings = await displaySettingsService.getDisplaySettingsWithPlaylistItems();
    res.status(200).json(displaySettings);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
async function update(req, res) {
  try {
    const displaySettings = await displaySettingsService.update(
      req.params.id,
      req.body
    );
    if (!displaySettings)
...
async function deleteById(req, res) {
  try {
    const displaySettings = await displaySettingsService.deleteById(
      req.params.id
    );
    if (!displaySettings)
      return res.status(404).json({ message: "Display settings not found" });
