...
exports.deleteSlideshow = async (req, res) => {
  ...
  console.log(id);
  try {
    // Fetch the slideshow with its associated media
    const slideshow = await Slideshow.findByPk(id, {
      include: [{ model: Media, as: "media" }],
    });

    // If slideshow doesn't exist, return a 404 response
    if (!slideshow) {
      return res.status(404).json({
        status: "fail",
    ...
    const mediaDeletionPromises = slideshow.media.map((media) =>
      media.destroy()
    );
    await Promise.all(mediaDeletionPromises);

    // Delete the slideshow itself
    ...
    res.status(500).json({
      status: "fail",
      message: "An error occurred while deleting the slideshow",
    });
  }
};
